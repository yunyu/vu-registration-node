...
if (process.argv.length <= 2) {
  ...
  saveCookie(process.env.VUNET_ID, process.env.VUNET_PW)
    ...
    .catch(err =>
      console.warn(err, 'Failed to log in, are your credentials properly set?'),
    );
...
function initRequestPromise(cookiesExist = true) {
  const j = rp.jar(new FileCookieStore(cookieJarPath));
  //   console.log(j);
  rp = rp.defaults({
    jar: cookiesExist ? j : true,
    headers: {
      'User-Agent':
...
async function saveCookie(username, password) {
  ...
  //   console.log(loginRes.request.uri);
  //   const contentRes = await rp.get({
  //     uri: 'https://acad.app.vanderbilt.edu/student-search/search',
  //     resolveWithFullResponse: true,
  //     followAllRedirects: true,
  //   });
  //   //   console.log(contentRes.body, contentRes.request.uri);

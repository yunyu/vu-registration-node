...
if (process.argv.length <= 2) {
  ...
  saveCookie(process.env.VUNET_ID, process.env.VUNET_PW)
    ...
    .catch(e =>
      console.warn("Failed to log in, are your credentials properly set?" + e)
    );
  ...
  register(jsonic(process.argv[3]))
    ...
    .catch(e => console.warn("Failed to register, are the cookies saved?" + e));
} else {
  printUsage();
}
...
function initRequestPromise() {
  if (fs.existsSync(cookieJarPath)) {
    const file_jar = rp.jar();
    file_jar._jar = tough.CookieJar.deserializeSync(
      JSON.parse(fs.readFileSync(cookieJarPath))
    );
    file_jar.setCookie = function(cookieOrStr, uri, options) {
      return file_jar._jar.setCookieSync(cookieOrStr, uri, options || {});
    };
    file_jar.getCookieString = function(uri) {
      return file_jar._jar.getCookieStringSync(uri);
    };
    file_jar.getCookies = function(uri) {
      return file_jar._jar.getCookiesSync(uri);
    };
    rp = rp.defaults({
      jar: file_jar,
      headers: {
...
async function saveCookie(username, password) {
  ...
  const commodoreId = querystring.parse(loginRes.request.uri.query).commodoreId;
  $ = await rp({
  ...
  await fs.writeFile(
    cookieJarPath,
    JSON.stringify(rp_jar._jar.serializeSync())
  );
  return result;
}
